...
public interface MakerAuditFileRepository extends JpaRepository<MakerAuditFile, Long> {

        // 제품 조회(분류 포함)
        @Query(nativeQuery = true, value = "SELECT * FROM maker_audit_file WHERE maker_name LIKE %:makerName% AND maker_address LIKE %:makerAddress% AND maker_person LIKE %:makerPerson% AND maker_phone LIKE %:makerPhone% AND class_name IN (:className) order by class_name desc")
        Optional<List<MakerAuditFile>> findByMakerNameContainingAndMakerAddressContainingAndMakerPersonContainingAndMakerPhoneContainingIgnoreCaseAndtempBusinessType(
